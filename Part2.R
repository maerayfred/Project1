...
example_build_url <- build_url(
  year="2022",
  get_vals=c("AGEP", "SEX", "PWGTP","GASP","GRPIP"),
  get_vals_subset = c("HHL=1", "HISPEED=0:10"),
  for_val = "state:10"
)
example_build_url


example_data2 <- httr::GET(example_build_url)
example_parsed_data <- fromJSON(rawToChar(example_data2$content))
...
category<-sapply(c("AGEP", "SEX","GASP","GRPIP"), function(x) is_numeric_variable(x,example_filtered_var_list))
print(category) 


example_parsed_tibble_converted<-example_parsed_tibble |>
  mutate(across(all_of(names(category[category])), as.integer))



print(example_parsed_tibble_converted)
...
summary_census(var2)
example_filtered_var_list$AGEP$`suggested-weight`
...
is_numeric_variable(x, example_filteSEXis_numeric_variable(x, example_filtered_var_list)
...
is_numeric_variable("AGEP", example_get_variable_list)
...
class(example_parsed_tibble$JWAP_fixed)

tib<-example_parsed_tibble
